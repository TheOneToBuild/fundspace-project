...
const EnhancedImageUploader = ({ onImageUploaded }) => {
  const {
    ...
    autoUpload: true,
    onUploadComplete: (urls) => onImageUploaded(urls[0])
  });

  const [dragOver, setDragOver] = useState(false);

  const handleFileInput = (e) => handleImageSelect(e);
  const handleDrop = (e) => {
    e.preventDefault();
    setDragOver(false);
    handleImageSelect(e);
  ...
  return (
    <div>
      <label 
        ...
        className={`cursor-pointer inline-flex items-center px-4 py-2 border border-slate-300 rounded-md shadow-sm text-sm font-medium transition-all ${uploading ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'text-slate-700 bg-white hover:bg-slate-50'} ${dragOver ? 'border-blue-400 bg-blue-50' : ''}`}
        onDrop={handleDrop}
